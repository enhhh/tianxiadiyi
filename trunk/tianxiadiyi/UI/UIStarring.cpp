...
bool UIStarring::init()
{
	...
	for (int i = 0; i < 3; i++)
	{
		const char* s = CCString::createWithFormat("SoulBeadAttributeValueLabel_%d", i+1)->getCString();
		soulBeadAttributeValueLabel[i] = dynamic_cast<UILabel*>(uiLayer->getWidgetByName(s));
	}
